...
async def send_job(client: httpx.AsyncClient, job_idx: int, idempotency_key: str) -> dict:
    payload = {
    ...
    try:
        resp = await client.post(GATEWAY_URL, json=payload, headers=headers, timeout=10.0)
        latency = (time.perf_counter() - start) * 1000  # ms
        return {"status_code": resp.status_code, "latency": latency, "success": resp.status_code == 202}
    except Exception as e:
        return {"status_code": 0, "latency": 0, "success": False, "error": str(e)}

async def main():
    print(f"🚀 Launching Stress Test: {TOTAL_REQUESTS} requests (Concurrency: {CONCURRENCY_LIMIT})...")
    ...
    print(f"Throughput:                   {TOTAL_REQUESTS / overall_duration:.2f} req/s")
