    ...
    async def execute(self, job: Job) -> dict[str, Any]:
        ...
        for step_order, step_name in PIPELINE_STEPS:
            if step_order in completed_steps:
                logger.info("pipeline.step_skipped", job_id=str(job.id), step=step_name, reason="already_checkpointed")
                continue
    ...
    async def _run_step(
        ...
        if step_name == "document_parsing":
            ...
            for idx, text in enumerate(chunks):
                ...
                embeddings.append({"chunk_index": idx, "text": text, "vector": normalized_vec})
            ...
            embeddings_data = previous_outputs.get("step_2_output", {}).get("embeddings", [])
